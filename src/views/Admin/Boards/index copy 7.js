...
import BoardGroupItem2 from 'components/idist/admin/boards/BoardGroupItem2';
...
const Boards = () => {
  ...
  const boardGroupsRedux = useSelector((state) => state.boardAdmin.boardGroups);
  const contentsRedux = useSelector((state) => state.boardAdmin.contents);

  const [sidebarName, setSidebarName] = useState('');
  const [addState, setAddState] = useState({}); // group: [boolean] , groupId: [groupId]
  const [renameState, setRenameState] = useState({}); // groupId: [id], boardId: [id]
  const [dropdownState, setDropdownState] = useState({}); // groupId: [id], boardId: [id]
  ...
  useEffect(() => {
    setContents({ ...contentsRedux, isGroup: !contentsRedux?.description });
  }, [contentsRedux]);

  const onClickAddGroupButton = () => {
    setAddState({ group: true });
    setSidebarName('');
    dispatch(resetBoardAdminContents());
  };

  const onClickGroupItem = ({ id }) => {
    dispatch(getBoardGroupInit({ id }));
  };
  const onClickGroupExpand = () => {};
  const onClickGroupAddButton = () => {};
  const onClickGroupMoreButton = ({ id }) => {
    setDropdownState((prev) => (prev?.groupId === id ? {} : { groupId: id }));
  };
  const onClickGroupDropdownRename = ({ id, name }) => {
    setSidebarName(name);
    setRenameState({ groupId: id });
  };
  const onClickGroupDropdownActivation = () => {
    // 팝업 띄우기
  };
  const addGroup = () => {};

  const renameGroup = ({ id, name, clubId }) => {
    dispatch(renameBoardGroupInit({ id, data: { name }, clubId }));
  };
  const activateGroup = () => {};
    ...
    <Suspense isLoading={false}>
      <div className="boards">
        ...
        <div className="boards-sidebar ">
          <button className="add-group text-h4" onClick={() => onClickAddGroupButton()}>
            {BVD.addGroup}
          </button>
          <DragDropContext onDragEnd={onDragEnd} onDragStart={onDragStart}>
            <Droppable droppableId={'group-droppable'}>
              {(dropProvider, snapshot) => (
                <div ref={dropProvider.innerRef} {...dropProvider.droppableProps}>
                  {/* 그룹 리스트 시작 */}
                  {_.sortBy(boardGroupsRedux, 'order').map((boardGroupItem, boardGroupIndex) => {
                    return (
                      <Draggable
                        key={boardGroupItem.id}
                        draggableId={String(boardGroupItem.id)}
                      ...
                      >
                        {(dragProvider, snapshot) => {
                          return (
                            ...
                            >
                              <BoardGroupItem2 boardGroup={boardGroupItem} />
                            </div>
                          );
                        }}
                      </Draggable>
        ...
        <div className="boards-contents-wrapper"></div>
      </div>
    </Suspense>
  );
};
