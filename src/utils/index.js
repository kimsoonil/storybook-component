      ...
      payload.actionList.map((action) => {
        if (action.useResponseData && responseData) {
          ...
          action.keyPairs.forEach((keyPair) => {
            payloadData[keyPair.payload] = responseData[keyPair.response];
          });
          // console.log({ type: action.type, payload: payloadData });
          return put({ type: action.type, payload: payloadData });
        }
        return put(action);
      })
    );
